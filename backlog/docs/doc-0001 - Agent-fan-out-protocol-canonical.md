...
> authoritative for this repo: an agent with only the checkout has the whole model, and the source
> is a loose unversioned file on one machine. It is copied, not summarised - if you find a
> compressed version of this anywhere in this repo, that version is wrong. When the source changes,
> the change is not finished until every consuming repo has been re-imported in the same commit.
