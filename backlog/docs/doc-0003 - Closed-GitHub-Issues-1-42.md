...
resolve them.** The full bodies and all 52 comments live in `[purged issue archive]`, which is
redacted with stable placeholders - see `[purged issue archive]`. To read one:

```bash
jq '.[] | select(.number == 32)' [purged issue archive]
```
